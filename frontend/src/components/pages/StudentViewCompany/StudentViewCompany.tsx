...
const StudentViewCompany: React.FC = () => {
  ...
  const response = useSelector(
    (state: any) => state.recruitment.recruitment?.response
  );
  const [majors, setMajors] = useState<Field[]>([]);
  const companies = useSelector(
    (state: any) => state.company.company?.companies
  );
  ...
  const applyToCompany = async (company: Company) => {
    ...
    const res = await axios.post(
      "http://localhost:8000/api/recruitment/create-recruitment",
      newRecruitment
    );
    console.log(res);
        ...
        <form onSubmit={searchSubmit}>
          ...
          <div className="cover">
            <div className="filter">
              <select
                className="majors filter--item filter--select"
                name="field"
              >
                Majors <i className="fas fa-sort-down"></i>
                <option className="majors-item" value={0} defaultChecked>
                  All
                </option>
                {majors.map((item) => (
                  <option className="majors-item" value={item.fieldID}>
                    {item.fieldName}
                  </option>
                ))}
              </select>
            ...
            >
              ...
              <div className="company-info">
                ...
                <p className="recruitment-description">
                  Major: <span></span>
                  {item.careerField.map((field, index) => (
                    <span>{field.fieldName + ", "}</span>
                  ))}
                </p>
                {item.applyPosition != null
                  ? item.applyPosition
                      .split("\n")
                      .slice(0, 2)
                      .map((item) => (
        ...
        <div className="paging">
          ...
          {pages.map((item) => (
            <NavLink
              ...
              to={"/student/companies?page=" + item}
            >
              {currentPage}
            </NavLink>
          ))}
          ...
          <div className="modal-container">
            <div className="company-infor">
              ...
              <a
                ...
                className="company-infor-detail js-website"
              >
                {company?.webSite}
              </a>
            </div>
            <div className="company-infor">
              ...
              <p className="company-infor-detail js-workplace">
                {company?.address}
              </p>
            </div>
            <div className="company-infor">
            ...
            <div className="btn-interact">
              {
                <button
                  ...
                  onClick={() => applyToCompany(company as Company)}
                >
                  apply
                </button>
              }
